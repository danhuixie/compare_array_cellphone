            ...
            while (recorderStatue == STATUE_RUNNING) {
                ...
                if (getRecorderConfig().isFileSaveFlag()) {
                    if (savefilename == null) {
                        savefilename = System.currentTimeMillis() + ".pcm";
                        savatime = System.currentTimeMillis();
                    }
                    RecorderImpl.write(thedata, getRecorderConfig().getContinuousFileSavePath(), savefilename);
                    if (System.currentTimeMillis() > savatime + 10000) {
                        zhuan(getRecorderConfig().getContinuousFileSavePath(), savefilename);
                        if (listener != null) {
                            File file = new File(getRecorderConfig().getContinuousFileSavePath() + "/" + savefilename);
...
//            long fileSize = data.size() * getRecorderConfig().getBufferSize() / 1024;
            ...
            if (getRecorderConfig().isFileSaveFlag()) {  //退出时保存最后一段文件并通知
                if (listener != null) {
    ...
    public static boolean zhuan(String filepath, String filename) {
        Log.e("RecorderImpl", "音频文件地址：" + filepath + "/" + filename);
        File file = new File(filepath + "/" + filename);
        String out = file.getName().replaceAll("\\.pcm", ".wav");
        String outfile = file.getParent() + "/" + out;
        if (file.exists()) {
            return copyWaveFile(filepath + "/" + filename, 16000, outfile);
...
//    }
    ...
    // 这里得到可播放的音频文件
    public static boolean copyWaveFile(String inFilename, int src_sample_rate, String outFilename) {
